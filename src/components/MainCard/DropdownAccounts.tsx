  ...
  return (
    <div className="dropdown-container">
      <div className="dropdown-container-title">
        <h4>
          YCabal account {currentAccount.registered ? "(linked)" : "(not linked)"}
        </h4>
        <KeysModal />
      </div>
      <select
        ...
        value={currentAccount.cabalAddress}
      >
        {walletAccounts.map((account) => (
          <option key={account?.cabalAddress} value={account?.cabalAddress}>
            {formatAccountString(account?.cabalAddress)}
          </option>
        ))}
      </select>
    </div>
